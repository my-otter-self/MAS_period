...
init 4 python in peMod_reminder:
    ...
    def addRecurringReminder(ev_label, delay, delta, latency):
        ...
        store.persistent._pe_mod_active_reminders[ev_label] = (
    ...
    def isReminderActive(ev_label):
        return ev_label in store.persistent._pe_mod_active_reminders

    def shouldTriggerReminder(ev_label):
        if not isReminderActive(ev_label):
            return False

        trigger, delta, latency = store.persistent._pe_mod_active_reminders[ev_label]
    ...
    def extendReminder(ev_label, keep_up=False):
        ...
        while True:
            ...
            if keep_up:
                if store.persistent._pe_mod_active_reminders[ev_label][0] <= now:
                    continue
                else:
                    break
            else:
    ...
    def stopReminder(ev_label):
        ...
        del store.persistent._pe_mod_active_reminders[ev_label]


init 7 python in peMod_reminder:
    ...
    def _handleMissedReminders():
        for ev_label in store.persistent._pe_mod_active_reminders.keys():
            trigger, delta, latency = store.persistent._pe_mod_active_reminders[ev_label]
